...
void clear() {
  for (int i = 0; i < DIGITS; i++) digitalWrite(DIGIT_PINS[i], LOW);
  for (int j = 0; j < SEGMENTS; j++) digitalWrite(SEGMENT_PINS[j], HIGH);
}

void setup() {
  for (int i = 0; i < DIGITS; i++) pinMode(DIGIT_PINS[i], OUTPUT);
  for (int i = 0; i < SEGMENTS; i++) pinMode(SEGMENT_PINS[i], OUTPUT);
  clear();
}
...
void update() {
  ...
  uint8_t segments = (~(NUMBERS[digitValue[digit]] | (0b00000001 * (digit == 0))));
  for (int i = 0; i < SEGMENTS; i++) digitalWrite(SEGMENT_PINS[i], (segments >> (SEGMENTS - i - 1)) & 1);
